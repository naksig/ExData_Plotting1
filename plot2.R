...
# Global Active Power as a line graph.
...
#  2.make plot: Plot the data as a line graph. Save the output as a PNG file plot2.
...
#  2b. change locale (as my PC's locale is non-English) 
...
#  2c. make the plot
     ...
     with(consumption, {
          plot(DateTime, GlobalActivePower, type="l",  #set as line graph
               xlab="",                                #no x label
               ylab="Gobal Active Power (kilowatts)",  #with y label
               main = ""                               #no title
          )
     })
